...
class CalculatorAddArgs {
  ...
  public write(output) {
    output.writeStructBegin("CalculatorAddArgs");
...
class CalculatorAddResult {
  ...
  public write(output) {
    output.writeStructBegin("CalculatorAddResult");
...
class CalculatorClient {
  ...
  public add(x: number, y: number, callback) {
  ...
  public recv_add (input, mtype, rseqid) {
    const noop = () => null;
    ...
    const result = new CalculatorAddResult();
    result.read(input);
    input.readMessageEnd();
...
class CalculatorProcessor {
  ...
  public process_add(seqid, input, output) {
    const args = new CalculatorAddArgs();
    args.read(input);
    input.readMessageEnd();
    if (this._handler.add.length === 2) {
      Q.fcall(this._handler.add, args.x, args.y).then((data) => {
          const result = new CalculatorAddResult({success: data});
      ...
      this._handler.add(args.x, args.y, (err, data) => {
        let result;
        if (err == null) {
          result = new CalculatorAddResult((err != null ? err : {success: data}));
          output.writeMessageBegin("add", Thrift.MessageType.REPLY, seqid);
        } else {
...
export { CalculatorProcessor as Processor }
export { CalculatorClient as Client }
